    ...
    public List<Comment> getComments(int recipeId){
        ...
        try(Connection con = ConnectionFactory.getConnection()){
            ...
            while(commentData.next()){
                /* Hämtar användarnamnet på den som skrev kommentaren */
                sql = "SELECT username FROM users WHERE id IN (SELECT user_id FROM comment_user WHERE comment_id = ?)";
                prepStmt = con.prepareStatement(sql);
                ...
                comments.add(new Comment(commentData.getString("content"), commentData.getString("date"), userData.getString("username")));
            }
        }catch(Exception e){
